...
const LiveDbPage: React.FC = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {/* Info banner */}
      <div className="flex items-start gap-3 p-4 bg-zinc-900/50 border border-white/10 rounded-xl backdrop-blur-md">
        <Server className="h-4 w-4 text-zinc-300 flex-shrink-0 mt-0.5" />
        <div>
          <p className="text-xs font-semibold text-zinc-200">What gets extracted</p>
      ...
      <div className="bg-zinc-900/50 backdrop-blur-md rounded-2xl border border-white/10 shadow-[0_0_20px_rgba(0,0,0,0.2)] overflow-hidden">
        <div className="border-b border-white/5 px-6 py-4 flex items-center gap-2 bg-black/20">
          ...
          <span className="text-sm font-semibold text-white">Oracle Connection</span>
        </div>
        ...
        {/* Feedback */}
        {error && (
          <div className="mx-6 mb-4 flex items-start gap-3 p-4 bg-red-950/30 border border-red-900/50 rounded-xl">
            <AlertCircle className="h-5 w-5 text-red-400 flex-shrink-0" />
            <p className="text-sm text-red-200 leading-relaxed">{error}</p>
          </div>
        )}
        {success && (
          <div className="mx-6 mb-4 flex items-start gap-3 p-4 bg-emerald-950/30 border border-emerald-900/50 rounded-xl">
